...
exports.store = async (file,params) => {
    ...
    try {
        ...
        `INSERT INTO banners(banner_image,banner_category) VALUES (?,?)`,[image,params.banner_category]
        )
    } catch (error) {
        message = error;      
    }
...
exports.update = async (id,file,params) => {
    ...
    try {
        const banner = await db.query(
            `UPDATE banners SET banner_image = ?,banner_category = ? WHERE id=${id}`,[image,params.banner_category]
        )
